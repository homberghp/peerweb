<?php
  /* $Id: groupresult2.php 1723 2014-01-03 08:34:59Z hom $ */
 ...
 */
function groupResultTable( $dbConn, $prjtg_id,$overall_criterium,$productGrade, $header, $criteria, $lang , $rainbow, $addlink=false ){
    echo getGroupResultTable( $dbConn, $prjtg_id,$overall_criterium,$productGrade, $header, $criteria, $lang , $rainbow, $addlink );
}
function getGroupResultTable( $dbConn, $prjtg_id,$overall_criterium,$productGrade, 
			      $header, $criteria, $lang , $rainbow , $addlink=false){
    global $root_url;
    $sql = "select afko,year,description,prj_id,milestone,prjm_id,grp_num,prjtg_id, coalesce(alias,'g'||grp_num) as alias"
    ...
    $result .= "\t<caption><span style='font-size:14pt;font-weight:bold;'>"
      ."peer assessment for project $afko $year, group <i>$alias</i><br/>\"$description\"<br/></span>"
      ."Prj_id/milestone=$prj_id/$milestone ($prjm_id) grp_num $grp_num ($prjtg_id) with average grade [$productGrade]<caption>\n";
    $result .= "<colgroup>\n";
    $result .= "\t<col width='8%'/>\n".
	"\t<col width='17%'/>\n".
	"\t<col width='13%'/>\n".
