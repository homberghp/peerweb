...
if ( !$isTutor ) {
    ...
    $sql = "select * from tutor join student_email on (userid=snummer) where tutor=?";
    $result = oneRecordQuery( $dbConn, $sql, [ $tutor_code ] );
    if ( $result === false ) {
    ...
    extract( $result, EXTR_PREFIX_ALL, 'tutor' );
}
...
//   $dbConn->log($tuhe_prjSel->getQuery());
    if ( !isSet( $_REQUEST[ 'snummer' ] ) && !isSet( $_SESSION[ 'snummer' ] ) ) {
        ...
        list( $_SESSION[ 'tuhe_prj_id' ], $_SESSION[ 'tuhe_milestone' ], $_SESSION[ 'tuhe_prjtg_id' ]) = explode( ':', $_SESSION[ 'tuhe_prj_id_milestone' ] = guessProjectFromStudent( $dbConn, $snummer ) );
        ...
        if ( !isInClass( $dbConn, $class_id, $snummer ) ) {
            ...
            list( $_SESSION[ 'tuhe_prj_id' ], $_SESSION[ 'tuhe_milestone' ]) = explode( ':', $_SESSION[ 'tuhe_prj_id_milestone' ] = guessProjectFromStudent( $dbConn, $snummer, $tuhe_prj_id, $tuhe_milestone ) );
        }
    }
    ...
    if ( isSet( $_REQUEST[ 'class_id' ] ) ) {
        ...
        if ( $class_id != $_SESSION[ 'class_id' ] ) {
            ...
            if ( !isInClass( $dbConn, $class_id, $snummer ) ) {
                ...
                list( $_SESSION[ 'tuhe_prj_id' ], $_SESSION[ 'tuhe_milestone' ], $_SESSION[ 'tuhe_prjtg_id' ]) = explode( ':', $_SESSION[ 'tuhe_prj_id_milestone' ] = guessProjectFromStudent( $dbConn, $snummer, $tuhe_prj_id, $tuhe_milestone ) );
            }
        }
    }
...
$page_opening = '';
$sql = "select * from student_email where snummer=?";
$resultSet = oneRecordQuery( $dbConn, $sql, [ $snummer ] );
if ( $resultSet === false ) {
