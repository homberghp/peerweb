...
if ( $validator_clearance ) {
    ...
    if ( hasCap( CAP_SYSTEM ) ) {
        $extra_constraint = '';
        $sql = "select * from project where prj_id=?";
        $pstm = $dbConn->prepare( $sql );
        $qr= $pstm->execute( [ $prj_id ] );
    } else {
        $extra_constraint = "and owner_id=?";
        $pstm = $dbConn->prepare( $sql );
        $qr= $pstm->execute( [ $prj_id, $peer_id ] );
    }
    if ( $qr === false ) {
...
$activity_project_checked = $resultSet->fetch()[ 'active_project_set' ] ? 'checked' : '';
