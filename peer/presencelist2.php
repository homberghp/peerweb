...
if ($resultSet === false) {
    ...
    $sql = "select coalesce(alias,'g'||apt.grp_num::text) as sgroup,st.snummer,size as grp_size,\n" .
            "roepnaam||coalesce('<br/>'||tussenvoegsel||'<br/>','<br/>')||achternaam as name," .
...
//    include '../templates/presencelist.html';
    ...
    $grpnum = 0;
    while (!$rs->EOF) {
        if ($grpnum != 0 && $grpnum!=$grp_num) {
        extract($rs->fields);
                echo "</div>\n";
        }
        if ($grpnum != $grp_num) {
            echo "<div class='presence-tableau g{$grp_num}'>\n";
        }
        echo "<div class='pbadge {$presence}' id='g{$grp_num}'><label for='s{$snummer}' >{$name}</label><button id='s{$snummer}'>{$face}</button></div>\n";
        $rs->moveNext();
