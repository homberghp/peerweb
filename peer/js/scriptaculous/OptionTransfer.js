...
function hasOptGroups(obj){
	if (obj!=null && obj.optgroups!=null) { return true; }
	return false;
	}
// -------------------------------------------------------------------
...
function moveSelectedOptionsStripPrefix(from,to) {
	// Move them over
	if (!hasOptions(from)) { return; }
	for (var i=0; i<from.options.length; i++) {
		var o = from.options[i];
		if (o.selected) {
			if (!hasOptions(to)) { var index = 0; } else { var index=to.options.length; }
			var splitResult= o.text.split(':');
			var newText=splitResult[1];
			to.options[index] = new Option( newText,	o.value, true, true);
			}
		}
	// Delete them from original
	for (var i=(from.options.length-1); i>=0; i--) {
...
function OT_transferRight() { moveSelectedOptionsStripPrefix(this.left,this.right); this.update(); }
