...
if (isSet($_POST['grades']) && isSet($_POST['exam_event'])) {
    $_SESSION['groupSet'] = $groupSet = harvest($_POST['grades']);
    $showInput = (count($groupSet) === 0);
} else if (isSet($_POST['grade']) && isSet($_POST['cand']) && isSet($_POST['commit'])) {
    $gc = count($_POST['grade']);
    $groupSet = array();
    $exam_event=$_SESSION['exam_event'];
    for ($c = 0; $c < $gc; $c++) {
        $groupSet[$_POST['cand'][$c]] = $_POST['grade'][$c];
    }
    $cands = implode(',', array_keys($groupSet));
...
if (count($groupSet)) {
    ...
    for (; !$resultSet->EOF; $resultSet->moveNext()) {
        ...
        $rows .="<tr><td><input type='hidden' name='cand[]' value='$snummer'/>$snummer</td><td>{$achternaam}{$tussenvoegsel}</td>"
                . "<td>$roepnaam</td><td><input style='text-align:right' size='3' type='number' name='grade[]' min='0' max='10.0' step='0.1' value='{$groupSet[$snummer]}'/></td></tr>\n";
    }
    if ($rows != "") {
