...
if ( $isTutorOwner ) {
    if ( isSet( $_POST[ 'bmove' ] ) ) {
        if ( $_POST[ 'bmove' ] == 'Move' && isSet( $_POST[ 'members' ] ) ) {
            ...
            //$dbConn->log($sql);
            if ( $resultSet === false ) {
                echo ( "<br>Cannot read assessment_count with " . $sql . " reason " . $dbConn->errorInfo()[ 2 ] . "<br>");
                stacktrace( 1 );
                die();
            }
            // get target group id
            ...
            $resultSet = $dbConn->query( $sql );
            if ( $resultSet === false ) {
                echo ( "<br>Cannot execute with " . $sql . " reason " . $dbConn->errorInfo()[ 2 ] . "<br>");
                stacktrace( 1 );
                die();
            }
            ...
            if ( $resultSet === false ) {
                echo( "<br>Cannot update projecgroups with <pre>" . $sql . "</pre><br/>reason " . $dbConn->errorInfo()[ 2 ] . "<br>");
                stacktrace( 1 );
                //die();
            }
            //	    }
    ...
    if ( isSet( $_POST[ 'delete' ] ) && isSet( $_POST[ 'members' ] ) ) {
        ...
        $resultSet = $dbConn->exec( $sql );
        if ( $resultSet === false ) {
            print 'error deleting reason: ' . $dbConn->errorInfo()[ 2 ] . '<br> with' . $sql . '<br>';
...
$resultSet = $dbConn->query( $sql );
if ( ($row = $resultSet->fetch()) === false ) {
    print 'cannot get row count: ' . $dbConn->errorInfo()[ 2 ] . '<br> with' . $sql . '<br>';
}
...
$resultSet = $dbConn->query( $sql );
if ( ($row = $resultSet->fetch()) === false ) {
    print( "<br>Cannot get project data with <pre>$sql</pre>, cause" . $dbConn->errorInfo()[ 2 ] . "<br>" );
}
$filled_count = $row[ 'filled_count' ];
$filled = $row[ 'filled_count' ] ? 'filled' : '<span style=\'color:yellow;\'>not</span> filled';
...
if ( $isTutorOwner ) {
    ...
    while ( ($row = $resultSet->fetch()) !== false ) {
        $assessment_open = $assessment_open || ($row[ 'prj_milestone_open' ] == 't');
    }
}
