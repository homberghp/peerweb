...
if ( hasCap( CAP_SYSTEM ) && isSet( $_REQUEST[ 'owner_id' ] ) ) {
    ...
    $sql = "update project set owner_id=? where prj_id=?";

    $pstm = $dbConn->prepare( $sql );
    $pstm->execute( [ $owner_id, $prj_id ] );
}
if ( $validator_clearance ) {
    if ( isSet( $_POST[ 'bsubmit' ] ) ) {
        if ( isSet( $_POST[ 'comment' ] ) )
            ...
            $comment = '';
        if ( $_POST[ 'bsubmit' ] == 'New' && isSet( $_POST[ 'afko' ] ) && ($_POST[ 'afko' ] != '') && isSet( $_POST[ 'project_description' ] ) && isSet( $_POST[ 'year' ] ) ) { // new
        ...
        } else if ( $_POST[ 'bsubmit' ] == 'Update' && isSet( $_POST[ 'prj_id' ] ) && isSet( $_POST[ 'afko' ] ) && isSet( $_POST[ 'project_description' ] ) && isSet( $_POST[ 'year' ] ) ) { // new
            $description = $_POST[ 'project_description' ];
            $afko = trim( $_POST[ 'afko' ] );
            $year = $_POST[ 'year' ];
    ...
    if ( $pstm === false ) {
        die( "<br>Cannot project data with <pre>$sql</pre> {$dbConn->errorInfo()[ 2 ]}<br>" );
    }
...
$sql = "select count(prj_id) as active_project_set from activity_project where prj_id=?";
$pstm = $dbConn->prepare( $sql );
if ( $pstm->execute( [ $prj_id ] ) ===false) {
    die( "<br>Cannot activity_project data with <pre>$sql</pre> {$dbConn->errorInfo()[ 2 ]}<br>" );
}

if ( ($row = $pstm->fetch()) !== false )
    $activity_project_checked = $row[ 0 ];
