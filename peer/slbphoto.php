...
$slb_sql = "select roepnaam||coalesce(' '||tussenvoegsel||' ',' ')||achternaam||' ('||tutor||':'||userid||')' as slb_info\n"
        . " from tutor join student_email  s on (s.snummer=tutor.userid) where userid=?";
$pst = $dbConn->prepare( $slb_sql );

//echo "<pre>{$slb_sql}</pre>";
if ( $pst->execute( [ $slb ] ) !== false ) {
    extract( $pst->fetch() );
}
$year = date( 'Y' );
