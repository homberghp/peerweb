...
$query = "select snummer  as groupname from prj_grp natural join prj_tutor pt \n"
...
$query="select snummer as groupname,snummer as username from prj_grp natural join prj_tutor pt  \n".
  "where prjm_id=$prjm_id order by groupname";
 $sth=$dbh->prepare($query)
    or die "Cannot execute ",$sth->errstr(),"\n";
$sth->execute()
    or die "Cannot execute ",$sth->errstr(),"\n";
...
$query = "begin work;\n"
    ...
    ."\t select milestone,'$repospath/'||snummer, '$repos_name '||snummer, \n"
    ."false,'$url_tail/'||snummer,owner_id as owner,grp_num,prjm_id,prjtg_id\n"
    ." from prj_grp natural join all_prj_tutor where prjm_id=$prjm_id ;\n"
