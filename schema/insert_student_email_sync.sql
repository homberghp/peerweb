begin work;
CREATE OR REPLACE FUNCTION public.insert_student_email_sync()
 RETURNS trigger
  LANGUAGE plpgsql
  AS $function$
  DECLARE
  ...
  BEGIN
    ...
    VALUES ( new.snummer,new.achternaam,new.tussenvoegsel,new.voorletters,new.roepnaam,new.straat,new.huisnr,
	     ...
	     new.phone_gsm,new.phone_postaddress,new.faculty_id,new.hoofdgrp,new.active,null,new.land,new.studieplan,
	     new.geboorteplaats,new.geboorteland,new.voornamen,0)
	on conflict(snummer)
	do update set ( snummer,achternaam,tussenvoegsel,voorletters,roepnaam,straat,huisnr,
	   		pcode,plaats,email1,nationaliteit,cohort,gebdat,sex,lang,pcn,opl,phone_home,
			phone_gsm,phone_postaddress,faculty_id,hoofdgrp,active,land,studieplan,
			...
			EXCLUDED.phone_gsm,EXCLUDED.phone_postaddress,EXCLUDED.faculty_id,EXCLUDED.hoofdgrp,EXCLUDED.active,EXCLUDED.land,EXCLUDED.studieplan,
...
-- insert into student_email values(879418, 'Hombergh', 'van den', 'PJFJ', 'Twan', 'Kerboschstraat', '12',
--  	     		   '5913WH ', 'Venlo', 'a.vandenhombergh@fontys.nl', 'NL', 1955 ,'1955-03-18', 'M', 'NL', 879418,112,'+31 77 3200075', '+31 653 759 245', '+31 8850 79417', 47, 'SEBINL2017', true,877516,'NLD',864,
-- 			  'Venlo', 'NLD', 'Petrus Joseph Franciscus Johannes',411) returning *;


-- \x
-- select * from student_email where snummer in( 879416, 879417,879418);
-- rollback;
